...
SELECT nom_categ FROM categorie 
WHERE nb_points = 3 ; 
...
-- 5. Nom des habitants commençant par la lettre A et contenant la lettre R (3)
...
INNER JOIN fabriquer ON fabriquer.num_potion = absorber.num_potion
...
WHERE lib_potion = "Potion magique n°1" AND date_a LIKE "%52-02%";
...
SELECT COUNT(*) AS nb_hab FROM habitant WHERE num_village = 5 ;
...
-- 21.Nombre de louches de potion magique n°2 
-- (c'est le libellé de la potion) absorbées. (19)  
...
SELECT village.nom_village, COUNT(habitant.num_hab) AS nbre_habitant FROM village 
...
SELECT habitant.nom, COUNT(absorber.num_potion) FROM absorber
INNER JOIN habitant ON habitant.num_hab = absorber.num_hab
GROUP BY habitant.nom;
...
SELECT village.nom_village FROM resserre
